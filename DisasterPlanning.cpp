...
Vector<string> remove_city(string city, Vector<string> unprotectedCities) {
    if (unprotectedCities.contains(city)){
        unprotectedCities.remove(unprotectedCities.indexOf(city));
    }
    return unprotectedCities;
}

bool canBeMadeDisasterReadyRec(const HashMap<string, HashSet<string>>& roadNetwork,
                               int numCities,
                               HashSet<string>& supplyLocations,
                               Vector<string> unprotectedCities) {

    if (numCities == 0 && !unprotectedCities.isEmpty()){
        return false;
    }

    if (unprotectedCities.isEmpty()){
        return true;
    }

    Vector<string> unprotectedCopy = unprotectedCities;

    string city = unprotectedCities.front();
    HashSet<string> adjacentCities = roadNetwork[city];

    unprotectedCities = remove_city(city, unprotectedCities);

    for (string key: adjacentCities) {
        unprotectedCities = remove_city(key, unprotectedCities);

    }

    if (canBeMadeDisasterReadyRec(roadNetwork, numCities-1, supplyLocations, unprotectedCities)){
        supplyLocations += city;
        return true;
    }

    unprotectedCities = unprotectedCopy;

    for (string key: adjacentCities) {
        city = key;
        unprotectedCities = remove_city(city, unprotectedCities);

        for (string adjacent: roadNetwork[key]) {
            unprotectedCities = remove_city(adjacent, unprotectedCities);
...
bool canBeMadeDisasterReady(const HashMap<string, HashSet<string>>& roadNetwork,
                            ...
                            HashSet<string>& supplyLocations) {
    if (numCities < 0) {
        error("Number of cities must be greater than or equal to zero!");

    }

    Vector<string> unprotectedCities = roadNetwork.keys();

    return canBeMadeDisasterReadyRec(roadNetwork, numCities, supplyLocations, unprotectedCities);
}
...
ADD_TEST("Returns true if there are no cities and numCities is also equal to zero."){
    HashMap<string, HashSet<string>> map = {};
    HashSet<string> locations;
    EXPECT(canBeMadeDisasterReady(map, 0, locations));
}
